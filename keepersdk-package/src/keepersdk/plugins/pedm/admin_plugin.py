...
from ...authentication import notifications, endpoint, keeper_auth
...
class PedmAdminNotifications(notifications.BasePushNotifications):
    ...
    def on_messaged_received(self, message: Union[str, bytes]):
        self.logger.debug('Received KEPM Admin notification: %s', message)
    ...
    def get_connection_parameters(self) -> Optional[notifications.PushConnectionParameters]:
        if 'ROUTER_URL' in os.environ:
            ...
            router_url = f'https://{self.auth.keeper_endpoint.get_router_server()}'
        ...
        session_token = self.auth.auth_context.session_token
        ...
        encrypted_transmission_key = endpoint.encrypt_with_keeper_key(
            transmission_key, self.auth.keeper_endpoint.server_key_id)
        ...
        return notifications.PushConnectionParameters(url=url, headers=headers)
...
class PedmPlugin(IPedmAdmin):
    def __init__(self, loader: enterprise_loader.EnterpriseLoader):
        ...
        self.storage: admin_storage.IPedmStorage
        if isinstance(loader_storage, sqlite_enterprise_storage.SqliteEnterpriseStorage):
            self.storage = admin_storage.SqlitePedmStorage(loader_storage.get_connection, self._enterprise_id)
        else:
            self.storage = admin_storage.MemoryPedmStorage()
        self.loader = loader
        ...
        self._push_notifications = PedmAdminNotifications(self.loader.keeper_auth)
    ...
    def agent_key(self) -> bytes:
        if self._agent_key is None:
            enterprise_info = self.loader.enterprise_data.enterprise_info
            x1 = int.from_bytes(enterprise_info.tree_key[:16], byteorder='big', signed=False)
            x2 = int.from_bytes(enterprise_info.tree_key[16:], byteorder='big', signed=False)
            salt = (x1 ^ x2).to_bytes(length=16, byteorder='big', signed=False)
            ec_private_key = crypto.unload_ec_private_key(enterprise_info.ec_private_key)
        ...
        for collection_dto in get_collections():
            try:
                if collection_dto.collection_type in (1000, 1001, 1002):
                    collection_value = collection_dto.data
                else:
    ...
    def sync_down(self, *, reload: bool = False) -> None:
        ...
        while not done:
            ...
            sync_rs = auth.execute_router('pedm/sync_pedm_data', sync_rq, response_type=pedm_pb2.GetPedmDataResponse)
        ...
        if len(delete_agents) > 0:
            ...
            self.storage.collection_links.delete_links_by_objects(delete_agents)
            self.storage.agents.delete_uids(delete_agents)
        if len(delete_collections) > 0:
            task.add_collections(delete_collections)
            self.storage.collection_links.delete_links_by_subjects(delete_collections)
            self.storage.collection_links.delete_links_by_objects(delete_collections)
    ...
    def assign_policy_collections(
        ...
        status_rs = auth.execute_router('pedm/set_policy_collections', rq,
                                        response_type=pedm_pb2.PedmStatusResponse)
        self._need_sync = True
    ...
    def modify_policies(self, *,
        ...
        status_rs = auth.execute_router('pedm/modify_policy', rq, response_type=pedm_pb2.PedmStatusResponse)
        self._need_sync = True
        assert status_rs is not None
        return admin_types.ModifyStatus.from_proto(status_rs)
    ...
    def modify_deployments(self, *,
        ...
        status_rs = auth.execute_router('pedm/modify_deployment', request=mrq, response_type=pedm_pb2.PedmStatusResponse)
        assert status_rs is not None
        self._need_sync = True
        return admin_types.ModifyStatus.from_proto(status_rs)
    ...
    def modify_collections(self, *,
        ...
        while len(to_add) > 0 or len(to_update) > 0 or len(to_remove) > 0:
            ...
            status_rs = auth.execute_router('pedm/modify_collection', request=crq,
                                            response_type=pedm_pb2.PedmStatusResponse)
            assert status_rs is not None
    ...
    def get_collection_links(self, *, links: Iterable[admin_types.CollectionLink]) -> Iterable[admin_types.CollectionLinkData]:
        ...
        link_rs = auth.execute_router(
            'pedm/get_collection_links', request=link_rq, response_type=pedm_pb2.GetCollectionLinkResponse)
    ...
    def set_collection_links(
        ...
        status_rs = auth.execute_router('pedm/set_collection_links', request=clrq,
                                        response_type=pedm_pb2.PedmStatusResponse)
        assert status_rs is not None
        self._need_sync = True
        return admin_types.ModifyStatus.from_proto(status_rs)
    ...
    def modify_agents(self, *,
        ...
        status_rs = auth.execute_router('pedm/modify_agent', rq, response_type=pedm_pb2.PedmStatusResponse)
        self._need_sync = True
        assert status_rs is not None
        return admin_types.ModifyStatus.from_proto(status_rs)
    ...
    def modify_approvals(self, *,
        ...
        status_rs = auth.execute_router('pedm/approval_action', rq, response_type=pedm_pb2.PedmStatusResponse)
        self._need_sync = True
        assert status_rs is not None
        return admin_types.ModifyStatus.from_proto(status_rs)
