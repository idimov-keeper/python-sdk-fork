...
def scan_records(keeper_auth_context: keeper_auth.KeeperAuth):
    """
    Scan records for breached passwords.
...
def main():
    """
    Main entry point for the scan records script.
    Performs login and scans records for breached passwords.
    """
    keeper_auth_context, _ = login()
    
    if keeper_auth_context:
        scan_records(keeper_auth_context)
    else:
        print("Login failed. Unable to scan records.")
