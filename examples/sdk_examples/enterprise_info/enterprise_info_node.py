...
def display_enterprise_nodes(keeper_auth_context: keeper_auth.KeeperAuth):
    """
    Load and display enterprise nodes information including user, team, and role counts.
    ...
    try:
        ...
        print("Enterprise Nodes Information")
        print("=" * 100)
        print(f"{'Node Name':<40} {'Node ID':<12} {'Users':<10} {'Teams':<10} {'Roles':<10}")
        print("-" * 100)
        
        for node in enterprise.enterprise_data.nodes.get_all_entities():
            node_name = node.name if hasattr(node, 'name') and node.name else 'N/A'
            ...
            print(f"{node_name[:39]:<40} {node_id[:11]:<12} {user_count:<10} {team_count:<10} {role_count:<10}")
        
        print("=" * 100)
        
        total_nodes = len(list(enterprise.enterprise_data.nodes.get_all_entities()))
...
def main():
    """
    Main entry point for the enterprise info node script.
    Performs login and displays enterprise nodes information.
    """
    keeper_auth_context, _ = login()
    
    if keeper_auth_context:
        display_enterprise_nodes(keeper_auth_context)
    else:
