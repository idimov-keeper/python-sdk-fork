...
def display_enterprise_tree(keeper_auth_context: keeper_auth.KeeperAuth):
    ...
    try:
        ...
        print("Enterprise Node Tree Structure")
        print("=" * 80)
        
        root_node = enterprise.enterprise_data.root_node
        if root_node:
...
def main():
    """
    Main entry point for the enterprise info tree script.
    Performs login and displays enterprise node tree structure.
    """
    keeper_auth_context, _ = login()
    
    if keeper_auth_context:
        display_enterprise_tree(keeper_auth_context)
    else:
