...
def perform_logout(keeper_auth_context: keeper_auth.KeeperAuth):
    """
    Perform logout after user confirmation.
    
    Args:
        keeper_auth_context: The authenticated Keeper context.
    """
    print(f"\nLogged in as: {keeper_auth_context.auth_context.username}")
    ...
    if confirm == 'yes':
        ...
        print("Logout cancelled. Session still active.")
        keeper_auth_context.close()


def main():
    """
    Main entry point for the logout script.
    Performs login and then offers logout option.
    """
    keeper_auth_context, _ = login()
    
    if keeper_auth_context:
        perform_logout(keeper_auth_context)
    else:
        print("Login failed.")
