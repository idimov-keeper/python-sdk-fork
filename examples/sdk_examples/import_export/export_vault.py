...
def export_vault(keeper_auth_context: keeper_auth.KeeperAuth):
    ...
    try:
        export_data = {'records': [], 'folders': [], 'metadata': {'version': '1.0', 'export_type': 'keeper_sdk', 'record_count': 0, 'folder_count': 0}}
        ...
        for record_info in vault.vault_data.records():
            ...
            record_data = {'uid': record_info.record_uid, 'title': record_info.title, 'type': record_info.record_type if hasattr(record_info, 'record_type') else 'login', 'version': record_info.version}
            if isinstance(record, vault_record.PasswordRecord):
                record_data['fields'] = {'login': record.login, 'password': record.password, 'url': record.link, 'notes': record.notes}
                record_data['custom_fields'] = record.custom
            elif isinstance(record, vault_record.TypedRecord):
                record_data['fields'] = [{'type': field.field_type, 'label': field.label if hasattr(field, 'label') else '', 'value': field.value} for field in record.fields]
            export_data['records'].append(record_data)
        for folder in vault.vault_data.folders():
            export_data['folders'].append({'uid': folder.folder_uid, 'name': folder.name, 'type': folder.folder_type, 'parent_uid': folder.parent_uid})
        ...
        print(f"\n✓ Export completed! Output: {output_file}, Records: {export_data['metadata']['record_count']}, Folders: {export_data['metadata']['folder_count']}")
    except Exception as e:
...
def main():
    ...
    if keeper_auth_context:
        export_vault(keeper_auth_context)
    else:
        print("Login failed.")
