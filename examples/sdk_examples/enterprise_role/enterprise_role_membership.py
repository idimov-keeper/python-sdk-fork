...
def view_role_membership(keeper_auth_context: keeper_auth.KeeperAuth):
    """
    View role membership details for a specific role.
    ...
    try:
        ...
        role_search = input('Enter role name or ID: ').strip()
        ...
        if not role_search:
            ...
            if role_found:
                role_name = role_found.name if hasattr(role_found, 'name') and role_found.name else 'N/A'
                
                print(f"\nRole Membership for: {role_name}")
                print("=" * 100)
                
                role_users = list(enterprise.enterprise_data.role_users.get_links_by_subject(role_found.role_id))
                role_teams = list(enterprise.enterprise_data.role_teams.get_links_by_subject(role_found.role_id))
                ...
                if role_teams:
                    ...
                    print("-" * 100)
                    print(f"{'Team Name':<50} {'Team UID':<50}")
                    print("-" * 100)
                    
                    for role_team in role_teams:
                        ...
                        if team:
                            ...
                            print(f"{team_name[:49]:<50} {role_team.team_uid[:49]:<50}")
                else:
                    print("\nNo teams assigned to this role")
                
                print("=" * 100)
            else:
                print(f'\nNo role found matching: "{role_search}"')
...
def main():
    """
    Main entry point for the enterprise role membership script.
    Performs login and displays role membership details.
    """
    keeper_auth_context, _ = login()
    
    if keeper_auth_context:
        view_role_membership(keeper_auth_context)
    else:
