...
class BiometricUnregisterCommand(BiometricArgparseCommand):
    ...
    def execute(self, context: KeeperParams, **kwargs):
        ...
        def _unregister():
            ...
            if not kwargs.get('confirm') and not self._get_user_confirmation(username):
                print("Operation cancelled by user")
                return

            rp_id = self._get_rp_id_from_server(context)
            ...
            verification_success = not self._check_biometric_flag(username)
            
            if delete_success and not verification_success:
                delete_success = self._delete_biometric_flag(username)
                verification_success = not self._check_biometric_flag(username)

            self._report_unregister_results(username, delete_success and verification_success, cleanup_success)
    ...
    def _get_rp_id_from_server(self, context: KeeperParams) -> Optional[str]:
        """Get RP ID from server authentication options"""
        try:
            auth_options = self.client.generate_authentication_options(context, 'login')
    ...
    def _disable_server_passkeys(self, context: KeeperParams):
        ...
        try:
            ...
            if stored_credential_id:
                ...
                self._process_specific_passkey_result(passkey_result)
            else:
    ...
    def _disable_specific_passkey(self, vault: vault_online.VaultOnline, credential_id: str):
        """Disable a specific passkey by credential ID"""
        try:
            available_credentials = self.client.get_available_credentials(vault)
            ...
            if target_passkey:
                result = self.client.disable_passkey(vault, target_passkey['id'], target_passkey['credential_id'])
                return result
            else:
    ...
    def _process_specific_passkey_result(self, passkey_result):
    ...
    def _cleanup_local_credentials(self, rp_id: Optional[str] = None) -> bool:
