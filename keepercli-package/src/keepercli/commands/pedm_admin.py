...
class PedmUtils:
    ...
    def get_orphan_resources(pedm: admin_plugin.PedmPlugin) -> List[str]:
        resource_types = {CollectionType.OsBuild, CollectionType.Application, CollectionType.UserAccount, CollectionType.GroupAccount}
...
class PedmCommand(base.GroupCommand):
    def __init__(self):
        ...
        self.register_command(PedmDeploymentCommand(), 'deployment', 'd')
...
class PedmDeploymentCommand(base.GroupCommand):
    def __init__(self):
        super().__init__('Manage PEDM deployments')
        self.register_command(PedmDeploymentListCommand(), 'list', 'l')
        self.register_command(PedmDeploymentAddCommand(), 'add', 'a')
        self.register_command(PedmDeploymentUpdateCommand(), 'edit')
        self.register_command(PedmDeploymentDeleteCommand(), 'delete')
        self.register_command(PedmDeploymentDownloadCommand(), 'download')
...
class PedmDeploymentListCommand(base.ArgparseCommand):
    ...
    def execute(self, context: KeeperParams, **kwargs):
        ...
        for dep in plugin.deployments.get_all_entities():
            ...
            agents = [x.agent_uid for x in plugin.deployment_agents.get_links_by_subject(dep.deployment_uid)]
            if verbose:
                row.append(agents)
            else:
...
class PedmDeploymentDownloadCommand(base.ArgparseCommand):
    ...
    def execute(self, context: KeeperParams, **kwargs):
        ...
        plugin = context.pedm_plugin
        deployment = PedmUtils.resolve_single_deployment(plugin, kwargs.get('deployment'))
        host = next((host for host, server in constants.KEEPER_PUBLIC_HOSTS.items() if server == auth.keeper_endpoint.server), auth.keeper_endpoint.server)
...
class PedmAgentCommand(base.GroupCommand):
    def __init__(self):
        ...
        self.register_command(PedmAgentEditCommand(), 'edit', 'e')
        self.register_command(PedmAgentDeleteCommand(), 'delete')
...
class PedmAgentListCommand(base.ArgparseCommand):
    ...
    def execute(self, context: KeeperParams, **kwargs) -> Any:
        ...
        if verbose:
            ...
            auth = plugin.loader.keeper_auth
            rq = pedm_pb2.PolicyAgentRequest()
            rq.summaryOnly = False
            rs = auth.execute_router("pedm/get_policy_agents", rq, response_type=pedm_pb2.PolicyAgentResponse)
...
class PedmPolicyCommand(base.GroupCommand):
    def __init__(self):
        ...
        self.register_command(PedmPolicyAgentsCommand(), 'agents')
        self.register_command(PedmPolicyAssignCommand(), 'assign')
...
class PedmPolicyListCommand(base.ArgparseCommand):
    ...
    def execute(self, context: KeeperParams, **kwargs) -> Any:
        ...
        for policy in plugin.policies.get_all_entities():
            ...
            collections = [x.collection_uid for x in plugin.storage.collection_links.get_links_by_object(policy.policy_uid)]
...
class PedmCollectionCommand(base.GroupCommand):
    def __init__(self):
        super().__init__('Manage PEDM collections')
        self.register_command(PedmCollectionListCommand(), 'list', 'l')
        self.register_command(PedmCollectionViewCommand(), 'view', 'v')
        self.register_command(PedmCollectionAddCommand(), 'add', 'a')
        self.register_command(PedmCollectionUpdateCommand(), 'update', 'u')
        self.register_command(PedmCollectionDeleteCommand(), 'delete')
        self.register_command(PedmCollectionConnectCommand(), 'connect')
        self.register_command(PedmCollectionDisconnectCommand(), 'disconnect')
        self.register_command(PedmCollectionWipeOutCommand(), 'wipe-out')
        self.default_verb = 'list'
        ...
        if isinstance(collection_type, int):
            ...
            for col in plugin.collections.get_all_entities():
                if col.collection_type != collection_type:
                    continue
                col_dict[col.collection_uid] = list(plugin.storage.collection_links.get_links_by_subject(col.collection_uid))
            ...
            if coll:
                ...
                if len(link_info) > 0:
                    ...
                    link_titles = list((f'{x.link_uid} ({pedm_shared.collection_link_type_to_name(x.link_type)})'
                                  for x in plugin.storage.collection_links.get_links_by_subject(collection_uid)))
                    if verbose:
...
class PedmApprovalCommand(base.GroupCommand):
    def __init__(self):
