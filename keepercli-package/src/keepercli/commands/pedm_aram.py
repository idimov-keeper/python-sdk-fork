...
from . import base, pedm_admin
from ..helpers import report_utils
from ..params import KeeperParams


class PedmReportCommand(base.GroupCommand):
    def __init__(self):
        super().__init__('Display PEDM  reports')
        self.register_command(PedmPolicyUsageReportCommand(), 'policy-usage', 'pu')
        self.register_command(PedmColumnReportCommand(), 'column', 'c')
        self.register_command(PedmEventReportCommand(), 'event', 'e')
        self.register_command(PedmEventSummaryReportCommand(), 'summary', 's')
...
class AuditMixin:
    ...
    @staticmethod
    def load_audit_metadata(auth: keeper_auth.KeeperAuth) -> None:
        if AuditMixin.syslog_templates is None:
            rq = {
                'fields': ['audit_event_type', 'report_field']
            }
            rs = auth.execute_router_json('pedm/get_audit_event_dimensions', rq)
...
class PedmColumnReportCommand(base.ArgparseCommand):
    ...
    def execute(self, context: KeeperParams, **kwargs) -> Any:
        ...
        if f_info is not None and f_info.protection == 'hash':
            ...
            coll_rs =  auth.execute_router(
                'pedm/get_audit_collections', coll_rq, response_type=pedm_pb2.AuditCollectionResponse)
            value_lookup: Dict[str, str] = {}
            assert coll_rs is not None
            ec_private = context.enterprise_data.enterprise_info.ec_private_key
            for v in coll_rs.values:
                value_uid = utils.base64_url_encode(v.valueUid)
                try:
                    decrypted_data = crypto.decrypt_ec(v.encryptedData, ec_private)
        ...
        for event in events:
            if 'admin_uid' in event:
                ...
                if isinstance(user_id, int):
                    user = context.enterprise_data.users.get_entity(user_id)
                    if isinstance(user, enterprise_types.User):
                        event['admin_uid'] = user.username
...
class PedmEventSummaryReportCommand(base.ArgparseCommand):
    def __init__(self):
        ...
        parser.add_argument('--aggregate', dest='aggregate', action='append', required=True,
    ...
    def execute(self, context: KeeperParams, **kwargs) -> Any:
        ...
        rq['order'] = order
        rs = auth.execute_router_json('pedm/get_summary_audit_report', rq)
        assert rs is not None
